...
void Timer0InterruptIsr(void)
{
	...
	{
          if (colorFlag == 0x02) {
		sprintf(countstr,"The red LED is on");      
          } else if (colorFlag == 0x04) {
		sprintf(countstr,"The blue LED is on");          
          } else if (colorFlag == 0x08) {
		sprintf(countstr,"The green LED is on");               
          }
            timeron=20; //i.e every 2 sec this block will execute
